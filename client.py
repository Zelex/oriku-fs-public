...
class DFSClient:
    ...
    def _shard_token(node_id: str, owner_fp: str,
        ...
        The secret is derived deterministically from the node_id —
        both client and node compute the same value independently.
        """
    ...
    async def _store_shard(self, node: dict, file_id: str,
        ...
        if direct_url and self._http:
            # Try local URL first (same LAN, avoids hairpin NAT), then
            # the public URL (UPnP external IP for remote clients).
            urls_to_try = []
            local_url = node.get("direct_url_local")
            if local_url and local_url != direct_url:
    ...
    async def _fetch_shards(self, file_id: str, shard_map: dict,
        ...
        node_direct_urls_local: Dict[str, str] = {}
        try:
            ...
            for n in nodes:
                if n.get("direct_url"):
                    ...
                    if n.get("direct_url_local"):
                        node_direct_urls_local[n["node_id"]] = n["direct_url_local"]
        except Exception:
            pass
        ...
        async def _fetch_direct(idx: int) -> Optional[Shard]:
            ...
            if not direct_url:
                return None
            # Try local URL first (same LAN), then public URL.
            urls = []
            local = node_direct_urls_local.get(node_id)
